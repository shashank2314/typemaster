...
//     setUserInput(value);
//     console.log("value",value,"userinput",userInput);
//     setCurrentIndex(value.length);
//     const words = value.trim().split(/\s+/).filter(word => word.length > 0);
//     setWordsTyped(words.length);
//     // Count words for word-based tests
...
//     const timeSpentSeconds = startTime ? (endTime.getTime() - startTime.getTime()) / 1000 : 0;
...
//     const wordsCount = userInput.trim().split(/\s+/).filter(word => word.length > 0).length;
//     const wpm = timeSpentSeconds > 0 ? (wordsCount / timeSpentSeconds) * 60 : 0;
//     console.log("userInput",userInput,"timeSpentSeconds",timeSpentSeconds,"correctChars",correctChars,"totalChars",totalChars,"incorrectChars",incorrectChars,"accuracy",accuracy,"wordsCount",wordsCount,"wpm",wpm);
...
//   const currentStats = {
//     wpm: startTime ? Math.round((wordsTyped / ((Date.now() - startTime.getTime()) / 1000)) * 60) || 0 : 0,
//     accuracy: userInput.length > 0 ? Math.round((calculateCorrectChars() / userInput.length) * 100) : 100
//   };
